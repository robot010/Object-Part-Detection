...
VGG_MEAN = [98.770,106.204, 113.380]
...
class tuned_FCN8VGG:
    ...
    def __init__(self, vgg16_npy_path=None):
        if vgg16_npy_path is None:
            ...
            # print path
            path = os.path.join(path, "../tensor_lstm_backup/tune_fcn8s_weights3.npy")
            vgg16_npy_path = path
    ...
    def build(self, rgb, train=False, num_classes=7, random_init_fc8=False,
              debug=False):
        """
        ...
        self.fc6 = self._fc_layer(self.pool5, name="fc6",num_classes=num_classes)

        if train:
            self.fc6 = tf.nn.dropout(self.fc6, 0.5)

        self.fc7 = self._fc_layer(self.fc6, name="fc7",num_classes=num_classes)
        if train:
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            if name == 'fc6':
                ...
                filt = self.get_fc_weight_reshape(name, [1, 1, 4096, 7])
            else:
    ...
    def _score_layer(self, bottom, name, num_classes):
        with tf.variable_scope(name) as scope:
            ...
            weights = self._variable_with_weight_decay(shape, name, stddev, w_decay,
                                                      decoder=True)
            conv = tf.nn.conv2d(bottom, weights, [1, 1, 1, 1], padding='SAME')
            # Apply bias
            conv_biases = self._bias_variable([num_classes], name)
            bias = tf.nn.bias_add(conv, conv_biases)
    ...
    def _upscore_layer(self, bottom, shape,
        ...
        with tf.variable_scope(name):
            ...
            weights = self.get_deconv_filter(f_shape, name)
    ...
    def _variable_with_weight_decay(self, shape, name, stddev, wd, decoder=False):
        ...
        initializer = tf.constant_initializer(value=self.data_dict[name][0],
                                       dtype=tf.float32)
    ...
    def _bias_variable(self, shape, name):
        ...
        initializer = tf.constant_initializer(value=self.data_dict[name][1], dtype=tf.float32)
